...
fn main() {
    // <2> main() can live happily at the beginning of a file
    let n: f32 = -42.42;
    ...
    println!(
        "{} -> [sign:{:01b}, exponent:{:08b}, mantissa:{:023b}] -> tbc",
        n, signbit, exponent, fraction
    );
    println!(
...
fn decode_f32_parts(sign: u32, exponent: u32, fraction: u32) -> (f32, f32, f32) {
    ...
    let _exponent = (exponent as i32) - BIAS; // <7> We need to do a bit of a type dance here. `exponent` must become an `i32` in case subtracting the `BIAS` results in a negative number. Then it needs to be cast as a `f32`, so that it can be used for exponentiation.
    println!("as is: {}", _exponent);

    // type converting just apply sign bit. so following bits doesn't reflect actual number.
    // println!("exponent {}", exponent);
    // println!("exponent as i8 {}", exponent as i8);
    // println!("exponent as i8 as i32 {}", exponent as i8 as i32);
    // let exponent = exponent as i8 as i32; // <7> We need to do a bit of a type dance here. `exponent` must become an `i32` in case subtracting the `BIAS` results in a negative number. Then it needs to be cast as a `f32`, so that it can be used for exponentiation.
    ...
    for i in 0..23_u32 {
        // <9> We provide a concrete type here to ensure that the bit patterns that are generated by the mask are defined
        ...
        if (one_at_bit_i & fraction) != 0 {
            // <11> Any non-zero result means that the bit is present within `fraction`
...
fn f32_from_parts(sign: f32, exponent: f32, mantissa: f32) -> f32 {
    ...
    // <12> This code cheats a bit by using `f32` values in intermediate steps. Hopefully it is a forgivable offense.
    sign * exponent * mantissa
}
...
mod tests {
    ...
    #[test]
    fn test_pow() {
        println!("{}", (-1.0_f32).powf(0 as f32));
        println!("{}", (-1.0_f32).powf(1 as f32));
        let u_num: u8 = 0b1111_1111;

        println!("u_num: {}", u_num);
        println!("u_num as i8: {:10b} {}", u_num as i8, u_num as i8);

        let i32_num: i32 = u_num as i32;
        println!("i32_num from u_num: {}", i32_num);
        let i32_num = u_num as i8 as i32;
        println!("i8_num: {}", i32_num);
    }
}
